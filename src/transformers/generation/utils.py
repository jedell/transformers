...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # TODO (joey) - Add GenerationMode and alpha/beta params
        if amateur_model is not None:
            print("Starting contrastive generation...")
            if generation_config.num_return_sequences > 1:
                raise ValueError(
                    "num_return_sequences has to be 1 when doing contrastive generate, "
                    f"but is {generation_config.num_return_sequences}."
                )
            if batch_size > 1:
                raise ValueError("contrastive generate is only supported for batch_size = 1")
            if not model_kwargs["use_cache"]:
                raise ValueError("contrastive generate requires `use_cache=True`")

            # 11. If the amateur model is an encoder-decoder, prepare its encoder outputs
            if amateur_model.config.is_encoder_decoder:
                inputs_tensor, model_input_name, amateur_model_kwargs = amateur_model._prepare_model_inputs(
                    inputs_tensor, amateur_model.generation_config.bos_token_id, amateur_model_kwargs
                )
                amateur_model_kwargs = amateur_model._prepare_encoder_decoder_kwargs_for_generation(
                    inputs_tensor, amateur_model_kwargs, model_input_name
                )
                model_kwargs["amateur_encoder_outputs"] = amateur_model_kwargs["encoder_outputs"]

            return self.contrastive_greedy_search(
                input_ids,
    ...
    def contrastive_greedy_search(
        self,
        input_ids: torch.LongTensor,
        amateur_model: "PreTrainedModel",
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        amateur_kv_indexing = (
            1
            if "bloom" in amateur_model.__class__.__name__.lower()
            or (
                amateur_model.config.architectures is not None
                and "bloom" in amateur_model.config.architectures[0].lower()
            )
            else 0
        )
        ...
        while True:
            ...
            if "amateur_past_key_values" in model_kwargs:
                prev_seq_len = model_kwargs["amateur_past_key_values"][0][amateur_kv_indexing].shape[-2]
                # `new_token_len` can be 1 or 2 (next token in assistant + last token picked by the larger model)
                new_token_len = input_ids.shape[1] - prev_seq_len
                amateur_inputs = input_ids[:, -new_token_len:]
                amateur_attn = torch.ones_like(input_ids)
                if amateur_model.config.is_encoder_decoder:
                    amateur_model_outputs = amateur_model(
                        decoder_input_ids=amateur_inputs,
                        decoder_attention_mask=amateur_attn,
                        past_key_values=model_kwargs["amateur_past_key_values"],
                        encoder_outputs=model_kwargs["amateur_encoder_outputs"],
                    )
                else:
                    amateur_model_outputs = amateur_model(
                        amateur_inputs,
                        attention_mask=amateur_attn,
                        past_key_values=model_kwargs["amateur_past_key_values"],
                    )
            else:
                if amateur_model.config.is_encoder_decoder:
                    ...
                    amateur_model_outputs = amateur_model(input_ids)            
            ...
            next_token_logits_amateur = amateur_model_outputs.logits[:, -1, :]

            alpha = torch.tensor([0.1])
            beta = 0.5

            cutoff = torch.log(alpha) + next_token_logits.max(dim=-1, keepdim=True).values
            diffs = (1 + beta) * next_token_logits - beta * next_token_logits_amateur

            cd_logits = diffs.masked_fill(next_token_logits < cutoff, -float('inf'))

            outputs.logits = cd_logits
            amateur_model_outputs.logits = cd_logits

            # expert_logits = next_token_logits
            # TODO (joey) - Get amateur logits via same process as above, look at assisteded_generation()
            # TODO (joey) - pass in alpha and beta parameters
            # TODO (joey) - remove logits processing for now and follow algo
            # TODO (joey) - add custom logits processors
            # cd_logits -> next_token_scores after processing, proceed as below

            # pre-process distribution, TODO (joey) REMOVE FOR NOW
            next_tokens_scores = logits_processor(input_ids, cd_logits) # replaced next_token_logits
            ...
            model_kwargs["amateur_past_key_values"] = amateur_model_outputs.past_key_values
